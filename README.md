...
- [📋 프로젝트 소개](#-프로젝트-소개)
    ...
    * [2. 현황 및 배경](#2-현황-및-배경)
    * [3. 핵심 기능](#3-핵심-기능)
        + [1. 실시간 지원자 상태 관리](#-실시간-지원자-상태-관리)
        + [2. 지원자 채용 단계 자동화 처리](#-지원자-채용-단계-자동화-처리)
        + [3. 면접 일정 관리 및 충돌 방지](#-면접-일정-관리-및-충돌-방지)
        + [4. 대면 면접 시 실시간 평가 공유](#-대면-면접-시-실시간-평가-공유)
        + [5. 유연한 채용 프로세스 체인](#-유연한-채용-프로세스-체인)
        + [6. 실시간 채용 단계 대시보드 & 파이프라인 시각화](#-실시간-채용-단계-대시보드--파이프라인-시각화)
        + [7. 실시간 화상 면접](#-실시간-화상-면접)
        + [8. 데이터 통계 및 분석 대시보드](#-데이터-통계-및-분석-대시보드)
        + [9. 대규모 트래픽 대응 대기열 시스템](#-대규모-트래픽-대응-대기열-시스템)
